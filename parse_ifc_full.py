...
def generate_brief_report(data, output_path):
    """Generate brief human-readable TXT report."""
    
    lines = []
    
    # Project Info
    lines.append("📋 Основная информация о проекте")
    lines.append("")
    pi = data['project_info']
    lines.append(f"Файл: {pi.get('name', 'N/A')}")
    lines.append(f"Дата создания: {pi.get('creation_date', 'N/A')}")
    lines.append(f"Схема: {pi.get('schema', 'N/A')}")
    lines.append(f"Объект: {pi.get('description', 'N/A')}")
    
    # Try to get address from project long_name, site or building
    address = 'N/A'
    if pi.get('long_name'):
        address = pi['long_name']
    elif 'site_long_name' in pi:
        address = pi['site_long_name']
    else:
        for b in data['buildings']:
            if b.get('long_name') and b['long_name'] != 'N/A':
                address = b['long_name']
                break
    ...
    # Building Structure
    lines.append("🏗️ Структура здания")
    lines.append("")
    if data['buildings']:
        b = data['buildings'][0]
        lines.append(f"Здание: {b.get('name', 'N/A')}")
    
    stories = data['stories']
    total_floors = len(stories)
    lines.append(f"Количество этажей: {total_floors} (включая подземный этаж, чердак и уровни крыши)")
    lines.append("")
    lines.append("Уровни:")
    
    # Group stories
    underground = [s for s in stories if s['elevation'] < 0 and 'подземн' in s.get('name', '').lower()]
    main_floors = [s for s in stories if 0 <= s['elevation'] < 55 and 'кровл' not in s.get('name', '').lower() and 'чердак' not in s.get('name', '').lower()]
    attic = [s for s in stories if 'чердак' in s.get('name', '').lower()]
    roof = [s for s in stories if 'кровл' in s.get('name', '').lower()]
    
    if underground:
        ug = underground[0]
        lines.append(f"-1: {ug.get('name', 'Подземный этаж')} ({ug['elevation']}м)")
    
    if main_floors:
        min_floor = min(s['elevation'] for s in main_floors)
        max_floor = max(s['elevation'] for s in main_floors)
        floor_nums = [s.get('name', '') for s in main_floors if s.get('name', '').isdigit()]
        if floor_nums:
            min_num = min(int(f) for f in floor_nums)
            max_num = max(int(f) for f in floor_nums)
            lines.append(f"{min_num}-{max_num}: Основные этажи (от {min_floor} до {max_floor}м)")
        else:
            lines.append(f"1-{len(main_floors)}: Основные этажи (от {min_floor} до {max_floor}м)")
    
    if attic:
        att = attic[0]
        lines.append(f"{len(main_floors) + 1}: Чердак ({att['elevation']}м)")
    
    if roof:
        roof_levels = ', '.join([f"{r['elevation']}м" for r in roof])
        start_idx = len(main_floors) + (1 if attic else 0) + 1
        end_idx = start_idx + len(roof) - 1
        lines.append(f"{start_idx}-{end_idx}: Крыша ({roof_levels})")
    
    lines.append("")
    
    # Element Counts
    lines.append("📊 Количество элементов")
    lines.append("")
    lines.append("Тип элемента\tКоличество")
    
    elements = data['elements_by_type']
    type_order = ["IfcWall", "IfcSlab", "IfcColumn", "IfcBeam", "IfcStair", 
                  "IfcRailing", "IfcRoof", "IfcOpeningElement", "IfcWindow",
                  "IfcDoor", "IfcBuildingElementProxy"]
    ...
    type_names = {
        ...
        "IfcRoof": "Крыши",
        "IfcOpeningElement": "Элементы проемов",
        "IfcWindow": "Окна",
        "IfcDoor": "Двери",
        "IfcBuildingElementProxy": "Вспомогательные элементы"
    }
    
    for t in type_order:
        if t in elements:
            count = len(elements[t])
            name = type_names.get(t, t)
            lines.append(f"{name} ({t})\t{count:,}")
    
    lines.append("")
    
    # Materials
    lines.append(f"🧱 Материалы ({len(data['materials'])} типа)")
    lines.append("")
    lines.append("Основные материалы включают:")
    
    materials = data['materials']
    # Categorize materials
    concretes = [m for m in materials if 'бетон' in m.lower() or 'B' in m.upper()]
    waterproofing = [m for m in materials if 'техноэласт' in m.lower() or 'planter' in m.lower() or 'гидро' in m.lower()]
    insulation = [m for m in materials if 'полистирол' in m.lower() or 'минеральн' in m.lower() or 'carbon' in m.lower() or 'утепл' in m.lower()]
    mortars = [m for m in materials if 'раствор' in m.lower() or 'стяжк' in m.lower() or 'цемент' in m.lower()]
    
    if concretes:
        concrete_samples = ', '.join(concretes[:5])
        if len(concretes) > 5:
            concrete_samples += f" и др."
        lines.append(f"Бетоны различных классов ({concrete_samples})")
    
    if waterproofing:
        wp_samples = ', '.join(waterproofing[:3])
        lines.append(f"Гидроизоляционные мембраны ({wp_samples})")
    
    if insulation:
        ins_samples = ', '.join(insulation[:3])
        lines.append(f"Утеплители ({ins_samples})")
    
    if mortars:
        mor_samples = ', '.join(mortars[:3])
        lines.append(f"Растворы и стяжки ({mor_samples})")
    
    other = [m for m in materials if m not in concretes + waterproofing + insulation + mortars]
    if other:
        other_samples = ', '.join(other[:5])
        if len(other) > 5:
            other_samples += f" и др."
        lines.append(f"Прочие материалы ({other_samples})")
    ...
    lines.append("")
    lines.append(f"Наборы свойств: {data['property_sets_count']:,}")
    lines.append(f"Наборы количеств: {data['quantity_sets_count']:,}")
    lines.append(f"Связи материалов: {data['material_relations']:,}")
    lines.append(f"Определения по свойствам: {data['property_definitions']:,}")
    
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    
    print(f"Brief report saved to: {output_path}")


def generate_detailed_excel(data, output_path):
    """Generate detailed Excel report with all elements, dimensions, materials, and quantities."""
    ...
    row = 1
    ws_summary.cell(row=row, column=1, value="📋 ОСНОВНАЯ ИНФОРМАЦИЯ").font = header_font
    row += 1
    
    pi = data['project_info']
    summary_data = [
        ("Файл:", pi.get('name', 'N/A')),
        ("Дата создания:", pi.get('creation_date', 'N/A')),
        ("Схема:", pi.get('schema', 'N/A')),
        ("Объект:", pi.get('description', 'N/A')),
        ("Адрес:", 'N/A'),
    ]
    ...
    row += 1
    ws_summary.cell(row=row, column=1, value="📊 КОЛИЧЕСТВО ЭЛЕМЕНТОВ ПО ТИПАМ").font = header_font
    row += 1
    
    ws_summary.cell(row=row, column=1, value="Тип элемента").font = header_font
    ws_summary.cell(row=row, column=2, value="Количество").font = header_font
    ...
    total_elements = 0
    for t, elems in elements.items():
        count = len(elems)
        total_elements += count
        name = type_names.get(t, t)
        ws_summary.cell(row=row, column=1, value=f"{name} ({t})")
        ws_summary.cell(row=row, column=2, value=count)
        row += 1
    
    row += 1
    ws_summary.cell(row=row, column=1, value="ВСЕГО ЭЛЕМЕНТОВ:").font = Font(bold=True)
    ws_summary.cell(row=row, column=2, value=total_elements).font = Font(bold=True)
    row += 2
    
    ws_summary.cell(row=row, column=1, value="🧱 МАТЕРИАЛЫ").font = header_font
    row += 1
    ws_summary.cell(row=row, column=1, value=f"Всего типов материалов: {len(data['materials'])}").font = Font(bold=True)
    row += 1
    
    ws_summary.cell(row=row, column=1, value="Материал").font = header_font
    ws_summary.cell(row=row, column=1).fill = header_fill
    row += 1
    
    for mat in data['materials']:
        ws_summary.cell(row=row, column=1, value=mat)
        row += 1
    
    row += 1
    ws_summary.cell(row=row, column=1, value="📁 ДОПОЛНИТЕЛЬНЫЕ ДАННЫЕ").font = header_font
    row += 1
    
    additional_data = [
        ("Наборы свойств:", data['property_sets_count']),
        ("Наборы количеств:", data['quantity_sets_count']),
        ("Связи материалов:", data['material_relations']),
        ("Определения по свойствам:", data['property_definitions']),
    ]
...
def main():
    ifc_file = "/workspace/К01_КР_П_R19.ifc"
    ...
    data = get_ifc_info(ifc_file)
    
    print("Generating brief report...")
    generate_brief_report(data, "/workspace/ifc_brief_report.txt")
    
    print("Generating detailed Excel report...")
    generate_detailed_excel(data, "/workspace/ifc_detailed_report.xlsx")
